...
import p2eAbi from '@/artifacts/contracts/NbaPlay.sol/NbaPlay.json'
...
const getPlayers = async(gameId: number): Promise<void> => {
  const contract = await getEthereumContracts();
...
const createGame = async (stake: number, totalPower: number): Promise<void> => {
  if (!ethereum) {
...
const joinGame = async (gameId: number, power: number, stake: number): Promise<void> => {
  if (!ethereum) {
    reportError('Please install a browser provider')
    return Promise.reject(new Error('Browser provider not installed'))
  }
...
const sendMoney = async() => {
  try{
    const contract = await getEthereumContracts()
    tx = await contract.sendMoney({ value: toWei(Number(0.001)) });
    await tx.wait()
...
const pickWinner = async(gameId: number) => {
  try{
    const contract = await getEthereumContracts()
    tx = await contract.getWinner(gameId);
    await tx.wait()
